...
namespace CDWKS.RevitAddon.Indexer2011
{
    ...
    public class IndexerCommand : IExternalCommand
    {
        ...
        public void Initialize()
        {
            ...
            ConnectionString =
                @"metadata=res://*/FileModel.csdl|res://*/FileModel.ssdl|res://*/FileModel.msl;provider=System.Data.SqlServerCe.3.5;provider connection string='DataSource=C:\ProgramData\Autodesk\Revit\Addins\2011\BIMXchangeAdmin.bundle\FileQueueDB.sdf'";
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            using(var login = new LoginForm())
            {
                ...
                if(login.DialogResult == DialogResult.OK)
                {
                    ...
                    const string str = "CDWKS.RevitAddon.Indexer2011.dll";
                    var str1 = path.Substring(0, path.Length - str.Length);
                    var args = InstanceId.ToString() + " 2011";
        ...
        private void IndexNext()
        {
            using (var model = new FileQueueEntities(ConnectionString))
            {
                ...
                Func<FileQueue, bool> filter = f => f.Status == Enums.IndexStatus.Queued.ToString()
                                                    & f.RevitVersion == Constants.RevitVersion
                ...
                using (var client = new AutodeskFileServiceClient(Binding, Endpoint))
                {
                        ...
                        items.Select(item => new AddTypeToFileRequest(item, indexedRevitFamily.Name, Constants.OwnerId)))
                    {
                       var typeResponse = client.AddTypeToFile(itemRequest);
                        if(typeResponse.AddTypeToFileResult == Enums.IndexStatus.DataSyncFailed.ToString())
                        {
